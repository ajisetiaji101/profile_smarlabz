...
@section('content')
    <div class="bg-white p-6 rounded-xl shadow-lg">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-2xl font-bold text-gray-800">Daftar Postingan Blog</h3>
            <a href="{{ route('dashboard.blogs.create') }}" class="bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-indigo-700 transition-colors duration-300">
        ...
        @if ($blogs->isEmpty())
            <div class="text-center py-10">
                <p class="text-gray-500">Belum ada postingan blog. Silakan buat postingan baru.</p>
            </div>
        @else
                        ...
                        @foreach ($blogs as $blog)
                            <tr class="border-b border-gray-200 hover:bg-gray-50">
                                <td class="py-3 px-6 text-left">
                                    @if ($blog->image)
                                        <img src="{{ Str::startsWith($blog->image, 'data:image') ? $blog->image : asset('storage/' . $blog->image) }}" alt="{{ $blog->title }}" class="w-20 h-16 object-cover rounded-md shadow-sm">
                                    @else
                                ...
                                <td class="py-3 px-6 text-left font-medium">{{ $blog->title }}</td>
                                <td class="py-3 px-6 text-left max-w-sm">{{ Str::limit($blog->content, 80) }}</td>
                                <td class="py-3 px-6 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center space-x-2">
                                        {{-- Tombol Edit --}}
                                        <a href="{{ route('dashboard.blogs.edit', $blog->id) }}" class="p-2 rounded-full text-indigo-600 hover:bg-indigo-100 transition-colors duration-200" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
