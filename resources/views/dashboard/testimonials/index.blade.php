...
@section('content')
    <div class="bg-white p-6 rounded-xl shadow-lg">
        <div class="flex justify-between items-center mb-6">
            <h3 class="text-2xl font-bold text-gray-800">Daftar Testimonial</h3>
            <a href="{{ route('dashboard.testimonials.create') }}" class="bg-indigo-600 text-white font-semibold py-2 px-4 rounded-lg hover:bg-indigo-700 transition-colors duration-300">
                <span class="flex items-center space-x-2">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z" clip-rule="evenodd" />
                    </svg>
                    <span>Buat Testimonial Baru</span>
                </span>
            </a>
        </div>
        ...
        @if ($testimonials->isEmpty())
            <div class="text-center py-10">
                <p class="text-gray-500">Belum ada testimonial. Silakan buat testimonial baru.</p>
            </div>
        @else
                ...
                <table class="min-w-full bg-white rounded-lg">
                    <thead class="bg-gray-100 text-gray-700 uppercase text-xs leading-normal">
                        <tr>
                            ...
                            <th class="py-3 px-6 text-left">Jabatan & Perusahaan</th>
                            <th class="py-3 px-6 text-left">Isi Testimonial</th>
                            ...
                            <tr class="border-b border-gray-200 hover:bg-gray-50">
                                <td class="py-3 px-6 text-left">
                                    @if ($testimonial->image)
                                        <img src="{{ Str::startsWith($testimonial->image, 'data:image') ? $testimonial->image : asset('storage/' . $testimonial->image) }}" alt="{{ $testimonial->name }}" class="w-20 h-16 object-cover rounded-md shadow-sm">
                                    @else
                                ...
                                <td class="py-3 px-6 text-left font-medium">{{ $testimonial->name }}</td>
                                <td class="py-3 px-6 text-left max-w-xs">
                                    {{ $testimonial->position }}
                                    @if ($testimonial->company)
                                        <br><span class="text-gray-500">{{ $testimonial->company }}</span>
                                    @endif
                                </td>
                                <td class="py-3 px-6 text-left max-w-sm">{{ Str::limit($testimonial->content, 80) }}</td>
                                <td class="py-3 px-6 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center space-x-2">
                                        {{-- Tombol Edit --}}
                                        <a href="{{ route('dashboard.testimonials.edit', $testimonial->id) }}" class="p-2 rounded-full text-indigo-600 hover:bg-indigo-100 transition-colors duration-200" title="Edit">
                                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
